...
public record PlayerClassData(WildMagicClass selectedClass, int level, int exp, int mana, int maxMana, int maxManaBonus, String slotOneAbility, String slotTwoAbility, String slotThreeAbility, String sorcererElement) {
    ...
    public SorcererElement sorcererElementEnum() {
        return SorcererElement.fromId(sorcererElement);
    }

    public PlayerClassData withSorcererElement(SorcererElement element) {
        return new PlayerClassData(selectedClass, level, exp, mana, maxMana, maxManaBonus, slotOneAbility, slotTwoAbility, slotThreeAbility, element.id());
    }
    ...
    public PlayerClassData withExp(int newExp) {
        return new PlayerClassData(selectedClass, level, Math.max(0, newExp), mana, maxMana, maxManaBonus, slotOneAbility, slotTwoAbility, slotThreeAbility, sorcererElement);
    }
    ...
    public PlayerClassData withMaxManaBonus(int bonus) {
        return new PlayerClassData(selectedClass, level, exp, Math.min(mana, maxMana + bonus), maxMana, bonus, slotOneAbility, slotTwoAbility, slotThreeAbility, sorcererElement);
    }
    ...
    public PlayerClassData withLevel(int newLevel) {
        ...
        return new PlayerClassData(selectedClass, clamped, exp, newMana, newMaxMana, maxManaBonus, slotOneAbility, slotTwoAbility, slotThreeAbility, sorcererElement);
    }

    public PlayerClassData withMana(int newMana) {
        return new PlayerClassData(selectedClass, level, exp, Math.clamp(newMana, 0, effectiveMaxMana()), maxMana, maxManaBonus, slotOneAbility, slotTwoAbility, slotThreeAbility, sorcererElement);
    }
    ...
    public PlayerClassData withActiveAbility(int slot, String abilityId) {
        ...
        return switch (slot) {
            case 0 -> new PlayerClassData(selectedClass, level, exp, mana, maxMana, maxManaBonus, normalizedAbilityId, slotTwo, slotThree, sorcererElement);
            case 1 -> new PlayerClassData(selectedClass, level, exp, mana, maxMana, maxManaBonus, slotOne, normalizedAbilityId, slotThree, sorcererElement);
            case 2 -> new PlayerClassData(selectedClass, level, exp, mana, maxMana, maxManaBonus, slotOne, slotTwo, normalizedAbilityId, sorcererElement);
            default -> this;
        };
    }
    ...
    public String serialize() {
        return (selectedClass == null ? "none" : selectedClass.id()) + ";" + level + ";" + exp + ";" + mana + ";" + maxMana + ";" + maxManaBonus + ";" + slotOneAbility + ";" + slotTwoAbility + ";" + slotThreeAbility + ";" + sorcererElement;
    }

    public static PlayerClassData deserialize(String raw) {
        if (raw == null || raw.isBlank()) return EMPTY;
        String[] parts = raw.split(";", -1);
        if (parts.length < 5 || "none".equals(parts[0])) return EMPTY;
        Optional<WildMagicClass> clazz = WildMagicClass.VALUES.stream()
                .filter(value -> value.id().equals(parts[0].toLowerCase(Locale.ROOT)))
                .findFirst();
        if (clazz.isEmpty()) return EMPTY;
        try {
            ...
            String sorcererElement = parts.length > 9 ? parts[9] : "";
