...
public class MagicWandItem extends Item {
    public MagicWandItem(Properties properties) { super(properties); }
    ...
    public InteractionResult use(Level level, Player player, InteractionHand hand) {
        ...
        if (!data.hasClass() || data.selectedClass() != WildMagicClass.SORCERER) {
            sp.sendSystemMessage(Component.literal("Только чародей может использовать волшебные палочки"));
        ...
        ((net.minecraft.server.level.ServerLevel) level).sendParticles(ParticleTypes.ENCHANT, sp.getX(), sp.getY() + 1.0D, sp.getZ(), 24, 0.4D, 0.6D, 0.4D, 0.02D);
